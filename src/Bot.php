...
class Bot
{
    ...
    public function parse()
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            ...
            Log::instance()->info("Method not allowed");
            exit();
        }
        ...
        if (strlen($entityBody) === 0) {
            ...
            Log::instance()->info("Missing request body");
            exit();
        }

        if (!hash_equals($this->sign($entityBody), $_SERVER['HTTP_X_LINE_SIGNATURE'])) {
            http_response_code(400);
            Log::instance()->info("Invalid signature value");
            exit();
        }
        ...
        if (!isset($data['events'])) {
            http_response_code(400);
            Log::instance()->info("Invalid request body: missing events property");
            exit();
        }
        return $data['events'];
    }
